...
def map_bus_error(
    ...
    *,
    ...
    `SOURCE_UNREADABLE` / `STREAM_UNREACHABLE`; an HTTP read failure after
    playback started is `STREAM_INTERRUPTED`.
    """
...
def _classify(gerror: GLib.Error, *, source_kind: SourceKind, was_playing: bool) -> FailureCode:
    ...
    if matches(resource, Gst.ResourceError.READ):
        if source_kind is SourceKind.HTTP and was_playing:
    ...
    if matches(resource, Gst.ResourceError.OPEN_WRITE) or matches(
        resource, Gst.ResourceError.WRITE
    ):
