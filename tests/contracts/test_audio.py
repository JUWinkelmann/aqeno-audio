...
def audio_fixtures(tmp_path_factory: pytest.TempPathFactory) -> AudioFixtures:
    ...
    stream_source = _render(root / "stream.flac", wave="sine", seconds=5, encoder="flacenc", freq=220)
...
class TestTransportStates:
    ...
    def test_play_pause_stop_cycle(self, engine: AudioEngine, audio_fixtures: AudioFixtures) -> None:
        recorder = Recorder(engine)
        engine.load(LocalFileSource(path=audio_fixtures.valid))
        recorder.wait_until(lambda: engine.state == TransportState.PAUSED)
...
class TestCapabilities:
    def test_known_length_file_reports_duration_and_seekable(
        ...
        recorder = Recorder(engine)
        if isinstance(engine, FakeAudioEngine):
            engine.force_next_capabilities(AudioCapabilities(seekable=True, duration=timedelta(seconds=2)))
        engine.load(LocalFileSource(path=audio_fixtures.valid))
        recorder.wait_until(lambda: engine.state == TransportState.PAUSED)
...
class TestContentFailures:
    ...
    def test_decode_failed(self, engine: AudioEngine, audio_fixtures: AudioFixtures) -> None:
        ...
        recorder = Recorder(engine)
        if isinstance(engine, FakeAudioEngine):
            engine.force_next_load_failure(FailureCode.DECODE_FAILED, "corrupt frame data (scripted)")
...
class TestStreamFailures:
    ...
    def test_stream_interrupted(self, impl: str, audio_fixtures: AudioFixtures) -> None:
        ...
        try:
            ...
            if isinstance(engine, FakeAudioEngine):
                engine.simulate_mid_playback_failure(FailureCode.STREAM_INTERRUPTED, "connection dropped")
...
def test_prepared_next_transitions_gaplessly(impl: str, audio_fixtures: AudioFixtures) -> None:
    ...
    try:
        ...
        # Armed before `play()`, exactly as an application must: `playbin3`
        # can emit `about-to-finish` almost immediately after preroll for a
        # short file (queue-level, not clock-level), so a next source queued
        # only once "close to the end" is too late.
        ...
        assert recorder.finished_count == 0, (
            "on_finished fired even though prepare_next() had armed a successor — "
            "exactly the audible gap ADR 0009 § 4a forbids"
        )

        non_playing = [state for state in recorder.states if state is not TransportState.PLAYING]
        if impl == "fake":
            # The scripted gapless switch fires no callback at all: the only
            # non-PLAYING states are the ordinary lead-in.
            assert non_playing == [TransportState.LOADING, TransportState.PAUSED]
        else:
            # The pipeline reaches STOPPED only once both clips are exhausted
            # — nothing in between the lead-in and that final STOPPED.
            assert non_playing == [TransportState.LOADING, TransportState.PAUSED, TransportState.STOPPED]
    finally:
        _close_engine(engine)
