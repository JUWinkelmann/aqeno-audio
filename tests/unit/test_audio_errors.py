...
MAPPED_CASES = [
    ...
    (RESOURCE, Gst.ResourceError.READ, SourceKind.LOCAL_FILE, True, FailureCode.SOURCE_UNREADABLE),
...
def test_explicit_mappings(
    ...
    failure = map_bus_error(
        gerror, "some debug text", source_kind=source_kind, was_playing=was_playing
    )
...
UNMAPPED_CASES = [
    ...
    (
        RESOURCE,
        Gst.ResourceError.NO_SPACE_LEFT,
        SourceKind.LOCAL_FILE,
        False,
...
def test_no_gstreamer_text_crosses_into_code() -> None:
    ...
    failure = map_bus_error(gerror, "debug", source_kind=SourceKind.LOCAL_FILE, was_playing=False)
